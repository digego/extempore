...
#ifndef NET_TS_IMPL_EXECUTOR_HPP
#define NET_TS_IMPL_EXECUTOR_HPP
...
public:
  ...
  impl(const Executor& e, const Allocator& a) NET_TS_NOEXCEPT
    : impl_base(false),
      ref_count_(1),
      executor_(e),
      allocator_(a)
  ...
  void dispatch(NET_TS_MOVE_ARG(function) f)
  {
    executor_.dispatch(NET_TS_MOVE_CAST(function)(f), allocator_);
  }

  void post(NET_TS_MOVE_ARG(function) f)
  {
    executor_.post(NET_TS_MOVE_CAST(function)(f), allocator_);
  }

  void defer(NET_TS_MOVE_ARG(function) f)
  {
    executor_.defer(NET_TS_MOVE_CAST(function)(f), allocator_);
  }

  type_id_result_type target_type() const NET_TS_NOEXCEPT
  {
    return type_id<Executor>();
  }

  void* target() NET_TS_NOEXCEPT
  {
    return &executor_;
  }

  const void* target() const NET_TS_NOEXCEPT
  {
    return &executor_;
  }

  bool equals(const impl_base* e) const NET_TS_NOEXCEPT
  {
    if (this == e)
      return true;
...
void executor::dispatch(NET_TS_MOVE_ARG(Function) f,
    const Allocator& a) const
{
  impl_base* i = get_impl();
  if (i->fast_dispatch_)
    system_executor().dispatch(NET_TS_MOVE_CAST(Function)(f), a);
  else
    i->dispatch(function(NET_TS_MOVE_CAST(Function)(f), a));
}

template <typename Function, typename Allocator>
void executor::post(NET_TS_MOVE_ARG(Function) f,
    const Allocator& a) const
{
  get_impl()->post(function(NET_TS_MOVE_CAST(Function)(f), a));
}

template <typename Function, typename Allocator>
void executor::defer(NET_TS_MOVE_ARG(Function) f,
    const Allocator& a) const
{
  get_impl()->defer(function(NET_TS_MOVE_CAST(Function)(f), a));
}

template <typename Executor>
Executor* executor::target() NET_TS_NOEXCEPT
{
...
template <typename Executor>
const Executor* executor::target() const NET_TS_NOEXCEPT
{
  return impl_ && impl_->target_type() == type_id<Executor>()
    ? static_cast<Executor*>(impl_->target()) : 0;
}
...
#endif // NET_TS_IMPL_EXECUTOR_HPP
