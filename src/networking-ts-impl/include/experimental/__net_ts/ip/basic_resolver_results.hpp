...
#ifndef NET_TS_IP_BASIC_RESOLVER_RESULTS_HPP
#define NET_TS_IP_BASIC_RESOLVER_RESULTS_HPP
...
public:
  ...
  basic_resolver_results(basic_resolver_results&& other)
    : basic_resolver_iterator<InternetProtocol>(
        NET_TS_MOVE_CAST(basic_resolver_results)(other))
  {
  }
      ...
      const std::string& host_name, const std::string& service_name)
  {
    ...
    while (address_info)
    {
      if (address_info->ai_family == NET_TS_OS_DEF(AF_INET)
          || address_info->ai_family == NET_TS_OS_DEF(AF_INET6))
      {
      ...
      const std::string& host_name, const std::string& service_name)
  {
    ...
    if (endpoints->Size)
    {
      ...
      for (unsigned int i = 0; i < endpoints->Size; ++i)
      {
        ...
        if (hints.ai_family == NET_TS_OS_DEF(AF_INET)
...
# endif // defined(NET_TS_WINDOWS_RUNTIME)
  ...
  size_type size() const NET_TS_NOEXCEPT
  {
    return this->values_ ? this->values_->size() : 0;
  }

  /// Get the maximum number of entries permitted in a results range.
  size_type max_size() const NET_TS_NOEXCEPT
  {
  ...
  void swap(basic_resolver_results& that) NET_TS_NOEXCEPT
  {
    if (this != &that)
    {
...
#endif // NET_TS_IP_BASIC_RESOLVER_RESULTS_HPP
