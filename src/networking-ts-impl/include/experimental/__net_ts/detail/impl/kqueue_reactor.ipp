...
#ifndef NET_TS_DETAIL_IMPL_KQUEUE_REACTOR_IPP
#define NET_TS_DETAIL_IMPL_KQUEUE_REACTOR_IPP
...
# define NET_TS_KQUEUE_EV_SET(ev, ident, filt, flags, fflags, data, udata) \
    ...
    registered_descriptors_mutex_(mutex_.enabled())
{
  ...
  NET_TS_KQUEUE_EV_SET(&events[0], interrupter_.read_descriptor(),
    ...
    std::experimental::net::v1::execution_context::fork_event fork_ev)
{
  if (fork_ev == std::experimental::net::v1::execution_context::fork_child)
  {
        ...
        state != 0; state = state->next_)
    {
      ...
      {
        NET_TS_KQUEUE_EV_SET(&events[0], state->descriptor_,
            EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0, state);
        NET_TS_KQUEUE_EV_SET(&events[1], state->descriptor_,
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data)
{
  ...
  NET_TS_HANDLER_REACTOR_REGISTRATION((
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, reactor_op* op)
{
  ...
  struct kevent events[1];
  NET_TS_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
  ...
  if (descriptor_data->op_queue_[op_type].empty())
  {
          ...
          || descriptor_data->op_queue_[except_op].empty()))
    {
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type])
      {
        ...
        NET_TS_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
        NET_TS_KQUEUE_EV_SET(&events[1], descriptor, EVFILT_WRITE,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, bool closing)
{
  ...
  if (!descriptor_data->shutdown_)
  {
    ...
    {
      struct kevent events[2];
      NET_TS_KQUEUE_EV_SET(&events[0], descriptor,
          EVFILT_READ, EV_DELETE, 0, 0, 0);
      NET_TS_KQUEUE_EV_SET(&events[1], descriptor,
    ...
    NET_TS_HANDLER_REACTOR_DEREGISTRATION((
          context(), static_cast<uintmax_t>(descriptor),
          reinterpret_cast<uintmax_t>(descriptor_data)));
...
void kqueue_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    if (ptr != &interrupter_)
    {
      ...
      {
      case EVFILT_READ:
        event_mask |= NET_TS_HANDLER_REACTOR_READ_EVENT;
        break;
      case EVFILT_WRITE:
        event_mask |= NET_TS_HANDLER_REACTOR_WRITE_EVENT;
        break;
      }
      if ((events[i].flags & (EV_ERROR | EV_OOBAND)) != 0)
        event_mask |= NET_TS_HANDLER_REACTOR_ERROR_EVENT;
      NET_TS_HANDLER_REACTOR_EVENTS((context(),
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    else
    {
          ...
          && descriptor_data->op_queue_[write_op].empty())
      {
        ...
        NET_TS_KQUEUE_EV_SET(&delete_events[0],
...
#undef NET_TS_KQUEUE_EV_SET

#include <experimental/__net_ts/detail/pop_options.hpp>

#endif // defined(NET_TS_HAS_KQUEUE)

#endif // NET_TS_DETAIL_IMPL_KQUEUE_REACTOR_IPP
