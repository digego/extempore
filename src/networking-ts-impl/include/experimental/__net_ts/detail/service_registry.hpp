...
#ifndef NET_TS_DETAIL_SERVICE_REGISTRY_HPP
#define NET_TS_DETAIL_SERVICE_REGISTRY_HPP
...
public:
  // Constructor.
  NET_TS_DECL service_registry(execution_context& owner);

  // Destructor.
  NET_TS_DECL ~service_registry();

  // Shutdown all services.
  NET_TS_DECL void shutdown_services();

  // Destroy all services.
  NET_TS_DECL void destroy_services();

  // Notify all services of a fork event.
  NET_TS_DECL void notify_fork(execution_context::fork_event fork_ev);
...
private:
  ...
  NET_TS_DECL static void init_key_from_id(
  ...
  NET_TS_DECL static bool keys_match(
  ...
  NET_TS_DECL static void destroy(execution_context::service* service);
  ...
  // create a new service object automatically if no such object already
  // exists. Ownership of the service object is not transferred to the caller.
  NET_TS_DECL execution_context::service* do_use_service(
      const execution_context::service::key& key,
      factory_type factory, void* owner);

  // Add a service object. Throws on error, in which case ownership of the
  // object is retained by the caller.
  NET_TS_DECL void do_add_service(
      const execution_context::service::key& key,
      execution_context::service* new_service);

  // Check whether a service object with the specified key already exists.
  NET_TS_DECL bool do_has_service(
...
#endif // NET_TS_DETAIL_SERVICE_REGISTRY_HPP
