...
#ifndef NET_TS_BUFFERS_ITERATOR_HPP
#define NET_TS_BUFFERS_ITERATOR_HPP
...
public:
...
#endif // defined(__GNUC__) && (__GNUC__ == 4) && (__GNUC_MINOR__ == 3)
  {
    ...
    new_iter.begin_ = std::experimental::net::v1::buffer_sequence_begin(buffers);
    new_iter.current_ = std::experimental::net::v1::buffer_sequence_begin(buffers);
    new_iter.end_ = std::experimental::net::v1::buffer_sequence_end(buffers);
...
private:
  ...
  void increment()
  {
    NET_TS_ASSERT(current_ != end_ && "iterator out of bounds");
    ++position_;
  ...
  void decrement()
  {
    NET_TS_ASSERT(position_ > 0 && "iterator out of bounds");
    --position_;
  ...
  void advance(std::ptrdiff_t n)
  {
    if (n > 0)
    {
      ...
      for (;;)
      {
        ...
        if (++current_ == end_)
        {
          NET_TS_ASSERT(n == 0 && "iterator out of bounds");
    ...
    {
      std::size_t abs_n = -n;
      NET_TS_ASSERT(position_ >= abs_n && "iterator out of bounds");
      for (;;)
      {
        // Check if the advance can be satisfied by the current buffer.
        ...
        if (current_ == begin_)
        {
          NET_TS_ASSERT(abs_n == 0 && "iterator out of bounds");
          current_buffer_position_ = 0;
          return;
        }
...
#endif // NET_TS_BUFFERS_ITERATOR_HPP
