...
scheme* scheme_wrapper_from_s7(s7_scheme* raw_sc)
{
    ...
    if (it != s_wrapperMap.end()) return it->second;
    return nullptr;
}
...
scheme* scheme_init_new()
{
    ...
    s7_define_function(sc->sc, "string-ref", lenient_string_ref,
                       2, 0, false, "(string-ref str i) returns str[i], #\\nul if out of bounds");
    ...
    s7_eval_c_string(sc->sc,
        "(let ((old-make-string make-string))"
        "  (set! make-string"
        "    (lambda args"
    ...
    s7_eval_c_string(sc->sc,
        "(define (emit-buffer->string x)"
        "  (if (output-port? x) (get-output-string x) x))");

    // bare (macro ...) form: (macro (name form) body...) or (macro name lambda)
    // form receives the ENTIRE s-expression including the macro name.
    // s7's define-macro destructures WITHOUT the macro name, so we cons it back.
    s7_eval_c_string(sc->sc,
        ...
        "            (form-arg (cadar args))"
        "            (body (cdr args)))"
        "        `(define-macro (,name . __ts_rest__)"
        "           (let ((,form-arg (cons ',name __ts_rest__)))"
        "             ,@body)))"
        "      (let ((name (car args))"
        "            (transformer (cadr args)))"
        "        `(define-macro (,name . __ts_rest__)"
        "           (,transformer (cons ',name __ts_rest__))))))");
    ...
    s7_eval_c_string(sc->sc,
        "(define (string->atom s) (with-input-from-string s read))");
    s7_eval_c_string(sc->sc,
        "(define (atom->string x) (object->string x))");
    ...
    s7_eval_c_string(sc->sc,
        "(define get-closure-code procedure-source)");
    ...
    s7_eval_c_string(sc->sc,
        "(define (make-closure code env) code)");

    // set-input-port and set-output-port
    s7_eval_c_string(sc->sc,
        "(define set-input-port set-current-input-port)");
    s7_eval_c_string(sc->sc,
        "(define set-output-port set-current-output-port)");
...
{
    if (!s7_is_string(key) && !s7_is_symbol(key)) return sc->F;
    ...
    for (pointer p = alist; s7_is_pair(p); p = s7_cdr(p)) {
        ...
        if (s7_is_symbol(ekey)) ekey_str = s7_symbol_name(ekey);
        else if (s7_is_string(ekey)) ekey_str = s7_string(ekey);
        else continue;
...
pointer _cons(scheme* sc, pointer a, pointer b, int immutable)
{
    ...
    if (immutable) s7_set_immutable(sc->sc, p);
    return p;
}
...
pointer mk_integer(scheme* sc, long long num) { return s7_make_integer(sc->sc, num); }
pointer mk_i64(scheme* sc, long long num) { return s7_make_integer(sc->sc, num); }
pointer mk_i32(scheme* sc, int num) { return s7_make_integer(sc->sc, num); }
pointer mk_i16(scheme* sc, short num) { return s7_make_integer(sc->sc, num); }
pointer mk_i8(scheme* sc, char num) { return s7_make_integer(sc->sc, num); }
pointer mk_i1(scheme* sc, bool num) { return s7_make_integer(sc->sc, num ? 1 : 0); }
pointer mk_real(scheme* sc, double num) { return s7_make_real(sc->sc, num); }
pointer mk_double(scheme* sc, double num) { return s7_make_real(sc->sc, num); }
pointer mk_float(scheme* sc, float num) { return s7_make_real(sc->sc, static_cast<double>(num)); }
...
pointer mk_closure(scheme* sc, pointer c, pointer e)
{
    ...
    return s7_eval(sc->sc, s7_cons(sc->sc, s7_make_symbol(sc->sc, "lambda"),
                                    c), e);
}
...
{
    if (s7_is_string(p)) return T_STRING;
    if (s7_is_number(p)) return T_NUMBER;
    if (s7_is_symbol(p)) return T_SYMBOL;
    if (s7_is_pair(p)) return T_PAIR;
    if (s7_is_character(p)) return T_CHARACTER;
    if (s7_is_vector(p)) return T_VECTOR;
    if (s7_is_c_pointer(p)) return T_CPTR;
    if (s7_is_boolean(p)) return T_NUMBER; // approximate
    if (s7_is_let(p)) return T_ENVIRONMENT;
...
int is_string(pointer p) { return s7_is_string(p); }
int is_number(pointer p) { return s7_is_number(p); }
int is_symbol(pointer p) { return s7_is_symbol(p); }
int is_pair(pointer p) { return s7_is_pair(p); }
int is_vector(pointer p) { return s7_is_vector(p); }
int is_character(pointer p) { return s7_is_character(p); }
int is_port(pointer) { return 0; } // s7 has ports but we don't expose them this way
int is_cptr(pointer p) { return s7_is_c_pointer(p); }
int is_cptr_or_str(pointer p) { return s7_is_c_pointer(p) || s7_is_string(p); }
int is_environment(pointer p) { return s7_is_let(p); }
int is_integer(pointer p) { return s7_is_integer(p); }
int is_real(pointer p) { return s7_is_number(p); }
int is_rational(pointer p) { return s7_is_ratio(p); }

int is_syntax(pointer p) { return s7_is_syntax(p); }
int is_proc(pointer p) { return s7_is_procedure(p) && !s7_is_c_pointer(p); }
int is_foreign(pointer p) { return s7_is_function(p); }

int is_closure(pointer p) { return s7_is_procedure(p) && !s7_is_function(p) && !s7_is_c_pointer(p); }
int is_macro(pointer) { return 0; } // TODO: s7_is_macro requires s7_scheme*
...
int is_promise(pointer) { return 0; }

int is_immutable(pointer p) { return s7_is_immutable(p); }
...
pointer pair_car(pointer p) { return s7_car(p); }
pointer pair_cdr(pointer p) { return s7_cdr(p); }
...
{
    if (s7_is_integer(p)) return s7_integer(p);
    if (s7_is_real(p)) return static_cast<int64_t>(s7_real(p));
    if (s7_is_ratio(p)) return s7_numerator(p) / s7_denominator(p);
    return 0;
}

double rvalue(pointer p)
{
    if (s7_is_real(p) && !s7_is_ratio(p) && !s7_is_integer(p)) return s7_real(p);
    if (s7_is_integer(p)) return static_cast<double>(s7_integer(p));
    if (s7_is_ratio(p)) return static_cast<double>(s7_numerator(p)) / s7_denominator(p);
    if (s7_is_number(p)) return s7_real(p);
    return 0.0;
}

int64_t i64value(pointer p) { return ivalue(p); }
int32_t i32value(pointer p) { return static_cast<int32_t>(ivalue(p)); }
int16_t i16value(pointer p) { return static_cast<int16_t>(ivalue(p)); }
int8_t i8value(pointer p) { return static_cast<int8_t>(ivalue(p)); }
bool i1value(scheme* sc, pointer p) { return p == sc->T; }
double r64value(pointer p) { return rvalue(p); }
float r32value(pointer p) { return static_cast<float>(rvalue(p)); }
long long charvalue(pointer p) { return s7_character(p); }
int is_integer_extern(pointer p) { return s7_is_integer(p); }
