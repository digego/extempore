...
(define fitslib
  (if (string=? "Linux" (sys:platform))
      (sys:open-dylib "libcfitsio.so.3")
      ;(sys:open-dylib "/usr/lib/x86_64-linux-gnu/libcfitsio.so.3")
      (print-error "tell me where to find the cfitsio dynamic library on your platform here!")))

(define wcslib
  (if (string=? "Linux" (sys:platform))
      (sys:open-dylib "libwcs.so.4.13.4")
      ;(sys:open-dylib "libwcs.so.4.8.3")
...
(definec fits-image-data-RGBA32-false
  (lambda (f:fitsfile* transparency:i8)
    (let ((status:i32* (salloc))
	  (err:i8* (salloc 40))
	  (dims:i32* (salloc))
