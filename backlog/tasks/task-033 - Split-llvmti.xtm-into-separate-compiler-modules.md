...
inference, closure conversion, generics, AOT support, and bind-form macros.
Split it along natural boundaries into separate files: caches.xtm (~2000 lines),
transforms.xtm (~300 lines), type-inference.xtm (~4600 lines),
closure-convert.xtm (~600 lines), generics.xtm (~1500 lines), aot.xtm (~500
lines), bind-forms.xtm (~2000 lines). Dependencies are mostly linear: caches →
transforms → type-inference → closure-convert → IR generation → bind-forms.
