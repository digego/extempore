...
--- there is no explicit AST type. Introduce a tagged AST representation (e.g.
vectors with tag fields or tagged lists) with accessor functions. This gives
each AST-consuming function an explicit contract and enables validation between
passes. Start with the output of first-transform and input to type-check, since
...
- [x] #1 AST node types defined with constructors and accessors (at minimum:
      let, lambda, if, call, var, lit, set!)
