...
Replaced deprecated `save-always` option with separate `actions/cache/restore`
and `actions/cache/save` steps.
...
2. Added new "Save LLVM cache" step after Build with condition
   `if: always() && steps.cache-llvm.outputs.cache-hit != 'true'`

This ensures cache is saved even when build/tests fail, preventing loss of
expensive LLVM compilation work.

Commit: 924be82f Run:
https://github.com/digego/extempore/actions/runs/20293247026
...
Windows cache was saved despite build failure, confirming the `always()`
