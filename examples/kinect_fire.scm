...
(load "libs/fluid2d_lib.scm")
(load "libs/kinect_lib.scm")
    	  ...
    	  (let ((idx (+ (+ ii 1)
    		...
    		(begin (pset! xs cnt (+ xoffset (i64tof ii)))
		       ...
		       (if (> cvar maxcvar) (set! maxcvar cvar))		       
    		       ...
    		       (pset! sizes cnt (+ 15.0 (* (- 1.0 cvar) 15.0)))
    		       (pset! states cnt 1) 
    		       (set! cnt (+ cnt 1))
    		       1)
    		1))))
...
(definec set-offset-x
  (lambda (val)
    (set! offset-x val)))

(definec set-offset-y
  (lambda (val)
    (set! offset-y val)))
...
(definec gl-loop
  (let (;(degree 0.0)
    ...
    (lambda ()
      ;; _skeletons-alive_ and _skeletons_ from kinect_lib.scm
      (dotimes (i 6)
...
(definec wind
  (lambda ()
    (let ((i 0))
      (dotimes (i 12)
	;(add-density-1 (* i 30) 10 300.0)
	(add-velocity-1 (* i 30) 5 (* 20.0 (- (random) .5)) 50.0))
      void)))


(definec update-borders
  (lambda ()
...
(begin (println 'go)
       (ipc:definec "fluid1" 'fluid-fsc-1)
       (ipc:definec "fluid2" 'fluid-fsc-2)       
       (println 'sleep)
       (sys:sleep 44100)
       (println 'startloop)
