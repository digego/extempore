  ...
  (lambda (b:i64 d:double* d0:double* velocx:double* velocy:double* velocz:double* dt:double N:i64)
	...
	(dotimes (j (- N 2))
	  ...
	  (dotimes (i (- N 2))
	    ...
	    (let ((tmp1 (* dtx (pref velocx (fluid-ix (+ i 1) (+ j 1) (+ k 1) N))))
		  (tmp2 (* dty (pref velocy (fluid-ix (+ i 1) (+ j 1) (+ k 1) N))))
		  (tmp3 (* dtz (pref velocz (fluid-ix (+ i 1) (+ j 1) (+ k 1) N))))
		  (x (- ifloat tmp1))
		  (y (- jfloat tmp2))
		  (z (- kfloat tmp3)))
...
(define smoke-signal
  (lambda (time)
    ...
    (add-density 2 5 11 (* (random) 50.0))
